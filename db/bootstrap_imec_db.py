...
def main():
    ...
    Index("ix_budgets_date", Budget.date).create(bind=engine, checkfirst=True)
    Index("ix_events_start_date", Event.start_date).create(bind=engine, checkfirst=True)
    Index("ix_news_date", News.date).create(bind=engine, checkfirst=True)
    Index("ix_legal_adoption_date", LegalInstrument.adoption_date).create(bind=engine, checkfirst=True)
    print("✅ Base IMEC initialisée avec succès.")
